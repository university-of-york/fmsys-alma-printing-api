...
What follows is the setup required to start `Fetch-Jobs` automatically upon logon, and a simple way for non-technical staff to re-launch the script if something untoward happens and the running instance needs to be restarted.
...
##### "Legacy", manual steps
These are included here "_just in case_".\
